...
EXPERIMENT_NAME = "exp002_wide_visit_first_last_delta"
EXPERIMENT_DIR = PROJECT_ROOT / "scripts" / "exp002"
OUTPUT_DIR = EXPERIMENT_DIR / "outputs"
BASELINE_SUMMARY_PATH = PROJECT_ROOT / "scripts" / "exp001" / "outputs" / "validation_summary.json"
...
def main() -> None:
    ...
    enriched_df, added_feature_names = shared.add_visit_summary_features(raw_df)
    enriched_test_df, _ = shared.add_visit_summary_features(test_df)
    feature_columns = shared.baseline_feature_columns(raw_df) + added_feature_names
    ...
    shared.save_json(
        OUTPUT_DIR / "feature_family_summary.json",
        {
            ...
            "feature_family": "Wide-visit first/last/delta summary features if _v* columns exist",
    ...
    wandb_summary = shared.log_experiment_to_wandb(
        ...
        extra_config={
            "feature_family": "wide_visit_first_last_delta",
