...
SELECTED_FAMILIES: list[tuple[str, Callable[[pl.DataFrame], tuple[pl.DataFrame, list[str]]]]] = [
...
def selected_family_names() -> list[str]:
    return [family_name for family_name, _ in SELECTED_FAMILIES]


def build_selected_features(df: pl.DataFrame) -> tuple[pl.DataFrame, list[str], list[str]]:
    enriched = df
...
def evaluate_target_detailed_generic(
    ...
    importance_getter: Callable[[Any], np.ndarray],
) -> dict[str, Any]:
