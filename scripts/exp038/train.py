...
EXPERIMENT_NAME = "exp038_xgboost_on_exp035_core"
EXPERIMENT_DIR = PROJECT_ROOT / "scripts" / "exp038"
OUTPUT_DIR = EXPERIMENT_DIR / "outputs"
BASELINE_SUMMARY_PATH = PROJECT_ROOT / "scripts" / "exp035" / "outputs" / "validation_summary.json"
FEATURE_FAMILY = "XGBoost model-class swap on exp035 curated feature stack"
...
def main() -> None:
    ...
    target_details = {
        semantic_target: exp035_core.evaluate_target_detailed_generic(
            ...
            importance_getter=lambda model: model.feature_importances_,
        )
    ...
    validation_summary["decision_reason"] = (
        "Accepted because XGBoost on the exp035 feature stack improved the official weighted C-index over exp035."
        if validation_summary["accepted"]
        else "Rejected because XGBoost on the exp035 feature stack did not improve the official weighted C-index over exp035."
    )
